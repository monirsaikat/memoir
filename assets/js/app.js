...
async function api(action, opts={}) {
  ...
  const res=await fetch(`api.php?action=${encodeURIComponent(action)}${opts.query||''}`, {...opts,headers,credentials:'same-origin'});
  const type=res.headers.get('content-type')||'';
  const data=type.includes('application/json')?await res.json():{ok:false,message:'The server returned an unexpected response'};
...
async function loadNote(id){
  ...
  if(window.matchMedia('(max-width: 760px)').matches) document.body.classList.add('editor-open');
}
...
$('#deleteNote').onclick=async()=>{if(!current||!confirm('Delete this note permanently?'))return;await api('delete-note',{method:'POST',body:JSON.stringify({id:current.id})});current=null;$('#editorView').classList.add('hidden');$('#emptyState').classList.remove('hidden');document.body.classList.remove('editor-open');await refreshList()};

function closeSidebar(){document.body.classList.remove('sidebar-open')}
$$('.nav-item').forEach(b=>b.onclick=()=>{filterFolder=b.dataset.folder??'';pinnedOnly=b.dataset.pinned==='1';$$('.nav-item,.folder-item').forEach(x=>x.classList.remove('active'));b.classList.add('active');$('#listTitle').textContent=pinnedOnly?'Pinned':'All notes';closeSidebar();refreshList()});
$('#folderList').addEventListener('click',e=>{const b=e.target.closest('.folder-item');if(!b)return;filterFolder=b.dataset.folder;pinnedOnly=false;$$('.nav-item,.folder-item').forEach(x=>x.classList.remove('active'));b.classList.add('active');$('#listTitle').textContent=b.querySelector('span').textContent;closeSidebar();refreshList()});

let searchTimer; searchInput.addEventListener('input',()=>{clearTimeout(searchTimer);searchTimer=setTimeout(refreshList,220)});
document.addEventListener('keydown',e=>{
  const mod=e.ctrlKey||e.metaKey, key=(e.key||'').toLowerCase();
  if(mod&&key==='k'){e.preventDefault();enableSearch();searchInput.focus();searchInput.select()}
  if(mod&&key==='n'){e.preventDefault();$('#newNote').click()}
  if(mod&&key==='s'){e.preventDefault();clearTimeout(saveTimer);saveNote()}
...
function openModal(id){const modal=$(id);modal.classList.remove('hidden');setTimeout(()=>modal.querySelector('input:not([type="hidden"]),button')?.focus(),0)} function closeModal(m){m.classList.add('hidden')}
