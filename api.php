...
case 'upload':
    ...
    if(@getimagesize($_FILES['image']['tmp_name'])===false) json_response(['ok'=>false,'message'=>'Invalid image file'],422);
    $ym=date('Y/m');$dir=__DIR__."/uploads/$ym";
    if(!is_dir($dir) && !mkdir($dir,0755,true) && !is_dir($dir)) json_response(['ok'=>false,'message'=>'Could not create upload folder'],500);
