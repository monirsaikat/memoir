    ...
    if (!$errors) {
        ...
        try {
            ...
            $schema = [
                ...
                "CREATE TABLE IF NOT EXISTS settings (id TINYINT UNSIGNED NOT NULL PRIMARY KEY DEFAULT 1, app_name VARCHAR(120) NOT NULL DEFAULT 'Memoir', mail_provider ENUM('smtp','brevo','phpmail') NOT NULL DEFAULT 'smtp', smtp_host VARCHAR(190) NULL, smtp_port INT NOT NULL DEFAULT 587, smtp_security VARCHAR(20) NOT NULL DEFAULT 'tls', smtp_user VARCHAR(190) NULL, smtp_pass TEXT NULL, smtp_from VARCHAR(190) NULL, brevo_api_key TEXT NULL, backup_enabled TINYINT(1) NOT NULL DEFAULT 1, backup_interval_hours SMALLINT UNSIGNED NOT NULL DEFAULT 24, backup_keep SMALLINT UNSIGNED NOT NULL DEFAULT 7, backup_last_at DATETIME NULL, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",
            ];
