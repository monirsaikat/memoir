    ...
    if (!$errors) {
        ...
        try {
            ...
            $schema = [
                ...
                "CREATE TABLE IF NOT EXISTS notes (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, folder_id INT UNSIGNED NULL, title VARCHAR(255) NOT NULL DEFAULT 'Untitled note', content LONGTEXT NOT NULL, color VARCHAR(20) NOT NULL DEFAULT '#FFFFFF', tags VARCHAR(500) NOT NULL DEFAULT '', icon VARCHAR(80) NOT NULL DEFAULT 'fa-note-sticky', is_pinned TINYINT(1) NOT NULL DEFAULT 0, deleted_at DATETIME NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, INDEX(folder_id), INDEX(updated_at), INDEX(deleted_at), CONSTRAINT fk_notes_folder FOREIGN KEY(folder_id) REFERENCES folders(id) ON DELETE SET NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",
